...
def build_rag_chain_from_categories(categories: List[str], vectorstores: dict):
    ...
    qa_prompt = ChatPromptTemplate.from_messages([
        ("system",  """당신은 개인 맞춤 영양제 코치입니다. 아래 사용자 정보를 분석해 사용자에게 필요한 성분과 피해야 할 성분을 알려주세요.
...
if submitted:
    ...
    with st.chat_message("ai"):
        with st.spinner("분석 중..."):
            ...
            if recommended:
                product_docs = search_products(recommended, top_k=5)
            ...
            # recommended = answer_json.get("recommended", [])
            # if recommended:
            #     query_str = ", ".join(recommended)   # 예: "비타민 D, 콜린"
            #     # 검색 결과는 현재 사용하지 않지만, query 전달을 위해 호출
            #     _ = search_products(query_str, top_k=5)
